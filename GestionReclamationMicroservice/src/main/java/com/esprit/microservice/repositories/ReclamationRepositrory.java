...
public interface ReclamationRepositrory extends JpaRepository<Reclamation, Integer>  {
	...
	public List<Reclamation> findReclamationsByOwnerId(@Param("id")int id);
	...
	@Query("select a from Reclamation a where (?1 is null or a.title = ?1) and (?2 is null or a.status= ?2) and (?3 is null or a.ownerId = ?3)and (?4 is null or a.date between ?4 and ?5)")
	 public List<Reclamation> findFiltredReclamation( @Param("title")String title , @Param("status")boolean status,@Param("ownerId")Integer ownerId,@Param("startDate")Date startDate,@Param("endDate")Date endDate);
}